...
Pulitzer.Prise.Change <- mutate(pulitzer, diff.2004.2014.to.1990.2003 = 
...
publiation.most.prizes.2014 <- filter(pulitzer, Pulitzer.Prize.Winners.and.Finalists..2004.2014 == 
...
biggest.decrease.with.wins <- filter(pulitzer, Pulitzer.Prize.Winners.and.Finalists..2004.2014 >= 5) %>% filter(Change.in.Daily.Circulation..2004.2013 == min(Change.in.Daily.Circulation..2004.2013)) %>% select(Newspaper)
...
# Which publisher had the greatest positive change in circulation from 2004 to 2013?
biggest.change.in.circ <- filter(pulitzer, Change.in.Daily.Circulation..2004.2013 == max(Change.in.Daily.Circulation..2004.2013)) %>% select(Newspaper)
